  ...
  <div id="addPicture">
    ...
    <a-form v-if="pictureForm.id" class="pictureForm" layout="inline" :model="pictureForm">
      <a-form-item label="名称">
        <a-input style="min-width:230px" v-model:value="pictureForm.name" placeholder="输入图片名称" />
      </a-form-item>
      <a-form-item label="简介">
        <a-textarea style="min-width:230px" v-model:value="pictureForm.introduction" placeholder="输入图片简介"
          :auto-size="{ minRows: 2, maxRows: 5 }" />
      </a-form-item>
      <a-form-item label="分类">
        <a-select style="min-width:230px" v-model:value="pictureForm.category" :options="categoryMap"
          placeholder="输入图片分类" />
      </a-form-item>
      <a-form-item label="标签">
        <a-select style="min-width:230px" mode="tags" v-model:value="pictureForm.tags" :options="tagMap"
          placeholder="输入图片标签" />
      </a-form-item>
    ...
    <a-modal v-model:open="openDesModel" title="对话框" @ok="handleOk">
      <a-form>
        <a-form-item>
          <a-textarea v-model:value="reviewForm.description"
            placeholder="请输入图片描述，例如：修改图片，遵循以下描述：一辆红色的破旧的自行车停在一条泥泞的小路上，背景是茂密的原始森林。描述你要修改的细节，越多越准确哟~"
